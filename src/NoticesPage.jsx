...
export default function NoticesPage({d,ctx,updated,notice=null}) {
  ...
  return <>
    ...
    {!notice&&ctx.kind==='class'&&classTeacherNotes[ctx.slug]?<a className="text-link" href={href('/teacher-notes')}>導師聯絡事項<CaretRight/></a>:null}{notice?<><article className="notice-detail"><p className="notice-date">{notice.date}</p>{expired?<p>{notice.expiredMessage||'此項申請已截止，以下保留供查閱。'}</p>:null}{notice.paragraphs?.map(p=><p key={p}>{p}</p>)}{notice.image?<figure className="notice-image"><a href={`${import.meta.env.BASE_URL}${notice.image.path}`} target="_blank" rel="noopener noreferrer" aria-label={`開啟${notice.image.alt}（另開分頁）`}><img src={`${import.meta.env.BASE_URL}${notice.image.path}`} alt={notice.image.alt} loading="lazy" width="1125" height="1100"/></a><figcaption>{notice.image.caption}，點圖可放大查看。</figcaption></figure>:null}{notice.link&&!expired?<a className="outline-button" href={notice.link.url} target="_blank" rel="noopener noreferrer">{notice.link.label}</a>:null}<small className="source">{notice.source}</small></article><a className="text-link" href={href('/notices')}>查看全部通知<CaretRight/></a></>:<>
